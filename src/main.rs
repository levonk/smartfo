...
fn install_pre_commit_hook(hooks_dir: &PathBuf, smartfo_binary: &PathBuf, force: bool) -> Result<()> {
    ...
    let hook_script = format!(
        r#"#!/bin/sh
# smartfo pre-commit hook - orchestrates all pre-commit checks via justfile
...
"#,
    ...
    if hook_path.exists() {
        ...
        if let Ok(content) = std::fs::read_to_string(&hook_path) {
            if content.contains("just pre-commit") || content.contains("smartfo git hook-client") || content.contains("smartfo git-hook-client") {
